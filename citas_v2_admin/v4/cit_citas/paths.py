...
@cit_citas.put("/cancelar", response_model=OneCitCitaOut)
...
async def paginado_cit_citas_mis_citas(
    current_user: Annotated[UsuarioInDB, Depends(get_current_active_user)],
    database: Annotated[Session, Depends(get_db)],
    cit_cliente_id: int,
):
    """Paginado de mis_citas, requiere el id del cliente, entrega desde hoy, al futuro y con estado PENDIENTE"""
    if current_user.permissions.get("CIT CITAS", 0) < Permiso.VER:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
    fecha = date.today()
    inicio_desde = datetime(
        year=fecha.year,
        month=fecha.month,
        day=fecha.day,
        hour=0,
...
@cit_citas.get("/mis_citas/curp", response_model=CustomPage[CitCitaOut])
async def paginado_cit_citas_mis_citas_curp(
    current_user: Annotated[UsuarioInDB, Depends(get_current_active_user)],
    database: Annotated[Session, Depends(get_db)],
    curp: str,
):
    """Paginado de mis_citas, requiere el curp del cliente, entrega desde hoy, al futuro y con estado PENDIENTE"""
    if current_user.permissions.get("CIT CITAS", 0) < Permiso.VER:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
    fecha = date.today()
    inicio_desde = datetime(
...
@cit_citas.get("/mis_citas/email", response_model=CustomPage[CitCitaOut])
async def paginado_cit_citas_mis_citas_email(
    current_user: Annotated[UsuarioInDB, Depends(get_current_active_user)],
    database: Annotated[Session, Depends(get_db)],
    email: str,
):
    """Paginado de mis_citas, requiere el email del cliente, entrega desde hoy, al futuro y con estado PENDIENTE"""
    if current_user.permissions.get("CIT CITAS", 0) < Permiso.VER:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
    fecha = date.today()
    inicio_desde = datetime(
