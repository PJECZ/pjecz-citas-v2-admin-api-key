"""
Cit Horas Disponibles v3, CRUD (create, read, update, and delete)
"""
...
from lib.exceptions import MyEmptyError, MyNotValidParamError
...
def get_cit_horas_disponibles(
    ...
    tiempos_bloqueados = []
    cit_horas_bloqueadas = get_cit_horas_bloqueadas(db=db, oficina_id=oficina_id, fecha=fecha).all()
    ...
    citas_ya_agendadas = {}
    for cit_cita in get_cit_citas_anonimas(db=db, oficina_id=oficina_id, fecha=fecha).all():
    ...
    if len(listado) == 0:
        raise MyEmptyError("No hay horas disponibles")
